...
- [x] **Step 3** — Port captures of the same scenes (26.2) → same plan § Step 3 (2026-09-18; `CREATEFLAT` + `UNDERSTORM` added to the port's `DevShot`, thin `WorldPresets.FLAT` superflat on the 26.2 `createFreshLevel` path, title-screen launch via `dev-relaunch-flat.sh`; evidence: `evidence-sym-0917-10-port-acdf` A·C·D·F 4/4, `-11-port-storm` 64/64, `-12-port-understorm` 41/41, `-13-port-shake` 41/41 — all zero refusals; `09-port-acdf` was the first thick-preset attempt. Cloud config identical (no diff). Step 4 parity notes in REPORT § Step 3: no rain in the port storm, darker/flatter storm base, SHAKE view +24 blocks. Tests 9/9 PASS)
- [x] **Step 4** — Comparison report original vs port → same plan § Step 4 (2026-09-18; all 11 scene views compared, side-by-side grid in `evidence-sym-0917-14-step4-grid/`; scene-by-scene table with severity + causes in REPORT § Step 4. **Item #1 VERIFIED**: the port never pushes the storm's local rain level to the vanilla level (original `WorldEffects.renderPost` sets `setRainLevel(manager.getRainLevel(...))`; port `WorldEffects` only reads it, kept at 0 by `ClientCloudManager`/`ServerCloudManager`) → no rain drops, no sky darkening, no rain sound. Other diffs: cloud layer/storm base render denser/flatter in the 26.2 port (mesh/LOD/alpha, config identical); distant storm (S4) a single blob vs textured mass+fog curtain. Lightning NOT a diff (port logged 77 strike/thunder/flash events). SHAKE view +24 blocks in the port)
