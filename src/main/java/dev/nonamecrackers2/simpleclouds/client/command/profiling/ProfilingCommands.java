...
public class ProfilingCommands
{
	...
	{
		dispatcher.register(ClientCommands.literal(SimpleCloudsMod.MODID).then(ClientCommands.literal("profiling").requires(stack -> true)
				.then(ClientCommands.literal("generator")
						.then(ClientCommands.argument("time", TimeArgument.time(1))
								.executes(ctx -> runGeneratorProfiler(ctx, ctx.getArgument("time", Integer.class)))
						)
	...
	private static int runGeneratorProfiler(CommandContext<FabricClientCommandSource> ctx, int iterations) throws CommandSyntaxException
	{
		Popup.createYesNoPopup(null, () ->
		{
			...
			Map<Identifier, CloudType> cachedCloudMap = ImmutableMap.copyOf(manager.getCloudTypes());
	...
	private static void acceptResults(ProfilingCloudGenerator.Results results)
	{
		...
		Map<Identifier, ProfilingCloudGenerator.CloudStats> individualStats = results.getIndividualStats();
		Consumer<Identifier> valueAcceptor = id -> {
		...
		main.setValue(Popup.createOptionListPopup(null, builder -> {
			for (Identifier id : individualStats.keySet())
				builder.addObject(Component.literal(id.toString()), id);
		}, valueAcceptor, 300, 100, mainMessage).alignLeft());
	}
	
	private static Component createIndividualResults(Identifier id, ProfilingCloudGenerator.CloudStats stats)
	{
