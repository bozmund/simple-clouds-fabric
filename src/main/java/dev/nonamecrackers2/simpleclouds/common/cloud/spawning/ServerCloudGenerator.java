...
public class ServerCloudGenerator extends CloudGenerator
{
	...
	{
		ListTag regionsTag = tag.getList("regions").orElseThrow();
		List<CloudRegion> regions = Lists.newArrayList();
		for (int i = 0; i < regionsTag.size(); i++)
		{
			CompoundTag regionTag = regionsTag.getCompound(i).orElseThrow();
			try {
...
//		System.out.println(regions);
		...
		this.ticksTillNextGen = tag.getInt("ticks_till_next_gen").orElseThrow();
	}
