...
{
	private final Identifier cloudTypeId;
	...
	public CloudRegion(Identifier cloudTypeId, Vec2 movementDirection, float maxSpeed, float accelerationFactor, float posX, float posZ, float radius, float rotation, float stretchFactor, int existsForTicks, int growTicks, int orderWeight)
	{
	...
	{
		this.cloudTypeId = buffer.readIdentifier();
	...
	{
		this.cloudTypeId = Identifier.read(tag.getString("id").orElseThrow()).resultOrPartial(e -> {
			throw new IllegalArgumentException(e);
		}).get();
		this.initialRadius = tag.getFloat("initial_radius").orElseThrow();
		this.movementDirection = PrimitiveHelper.vec2FromTag(tag.getCompound("movement_direction").orElseThrow());
		this.maxSpeed = tag.getFloat("max_speed").orElseThrow();
		this.accelerationFactor = tag.getFloat("acceleration_factor").orElseThrow();
		this.orderWeight = tag.getInt("order_weight").orElseThrow();
		CompoundTag vel = tag.getCompound("velocity").orElseThrow();
		this.velX = vel.getFloat("x").orElseThrow();
		this.velZ = vel.getFloat("z").orElseThrow();
		CompoundTag pos = tag.getCompound("pos").orElseThrow();
		this.posX = pos.getFloat("x").orElseThrow();
		this.posXO = this.posX;
		this.posZ = pos.getFloat("z").orElseThrow();
		this.posZO = this.posZ;
		this.radius = tag.getFloat("radius").orElseThrow();
		this.radiusO = this.radius;
		this.stretchFactor = tag.getFloat("stretch_factor").orElseThrow();
		this.stretchFactorO = this.stretchFactor;
		this.rotation = tag.getFloat("rotation").orElseThrow();
		this.rotationO = this.rotation;
		this.tickCount = tag.getInt("tick_count").orElseThrow();
		this.existsForTicks = tag.getInt("exists_for_ticks").orElseThrow();
		this.growTicks = tag.getInt("grow_ticks").orElseThrow();
	}
	
	public void toPacket(FriendlyByteBuf buffer)
	{
		buffer.writeIdentifier(this.cloudTypeId);
...
//		}
	...
	@Override
	public Identifier getCloudTypeId()
	{
