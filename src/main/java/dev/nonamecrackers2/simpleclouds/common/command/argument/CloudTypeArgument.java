...
public class CloudTypeArgument extends IdentifierArgument
{
	...
	private @Nullable List<Identifier> cloudTypes;
	...
	{
		Identifier loc = super.parse(reader);
	...
	public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder)
	{
		List<String> vals;
		if (this.source == null)
			vals = this.cloudTypes.stream().map(Identifier::toString).collect(Collectors.toList());
		else
			vals = Arrays.stream(this.source.getIndexedCloudTypes()).map(CloudType::id).map(Identifier::toString).collect(Collectors.toList());
