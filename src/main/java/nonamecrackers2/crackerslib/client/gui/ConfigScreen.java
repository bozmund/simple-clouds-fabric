...
public class ConfigScreen extends Screen
{
	...
	private static Map<String, Object> filterValues(String modid, ModConfig.Type type, String previousPath, Map<String, Object> values)
	{
		return values.entrySet().stream().map(entry -> {
			...
			// Fabric: no AddConfigEntryToMenuEvent bus event; all values included.
			return true;
	...
	protected void init()
	{
		if (this.list == null)
		{
			...
			// 26.2: updateSizeAndPosition(width, height, x, y) — argument order changed
			// from the old (x, y, width, height). Must run BEFORE buildList(): the
			// entries are sized from the list's row width while they are built.
			this.list.updateSizeAndPosition(this.width, this.height - 60, 0, 30);
	...
	public void extractRenderState(GuiGraphicsExtractor stack, int mouseX, int mouseY, float partialTicks)
	{
		...
		stack.centeredText(this.font, this.title.getString(), this.width / 2, TITLE_HEIGHT, 0xFFFFFF);
		boolean shiftDown = this.minecraft.options.keyShift.isDown();
		this.changePreset.setTooltip(Tooltip.create(this.getPresetTooltip(shiftDown)));
		...
		if (!this.children().stream().anyMatch(c -> !c.equals(this.list) && c.isMouseOver((double)mouseX, (double)mouseY)) && this.currentHoveredTooltip != null)
			stack.setTooltipForNextFrame(this.font, this.currentHoveredTooltip.toCharSequence(this.minecraft), mouseX, mouseY);
	}
