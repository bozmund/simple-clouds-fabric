...
//		this.sortType.sortList(this.children);
	...
	@Override
	public void render(GuiGraphicsExtractor stack, int x, int y, int width, int height, int mouseX, int mouseY, float partialTicks)
	{
		this.expand.setY(y + height / 2 - this.expand.getHeight() / 2);
		this.expand.extractRenderState(stack, mouseX, mouseY, partialTicks);
		stack.text(this.mc.font, this.displayName, x + 5 + (this.expand.getX() - x) + this.expand.getWidth(), y + height / 2 - this.mc.font.lineHeight / 2, 0xFFFFFFFF);
	}
